...
<p align="center">
  Project-scoped Railway CLI, built as a portable Go binary.<br>
...
Requires no runtime other than the released binary. No Railway CLI or Python needed.

```bash
curl -L -o railguey https://github.com/eidos-agi/railguey/releases/latest/download/railguey_linux_amd64
chmod +x railguey
```
...
```bash
git clone https://github.com/eidos-agi/railguey.git && cd railguey
go build -o railguey ./cmd/railguey
```
...
The native binary exposes the deployment-critical commands:

```bash
railguey status ~/repos/my-app
railguey deployments ~/repos/my-app cerebro --limit 5
railguey doctor ~/repos/my-app
railguey upload-source ~/repos/my-app web --message "$GIT_SHA"
```
...
The Go binary owns the commands below. The old Python implementation remains in
the repository only as unported compatibility reference and is not used by CI.

| Command | What it does |
|------|-------------|
| `railguey status` | Project, environment, and service identities |
...
1. `RAILWAY_TOKEN` is available from `.env.local`, `.env`, or CI environment
2. GitHub Actions deploy workflow exists

Every command takes a workspace parameter. Its token comes from `.env.local`, `.env`, or `RAILWAY_TOKEN` in CI.
...
The binary reads the workspace token, resolves the project and service, creates a gzipped source archive excluding `.git`, `.env*`, `node_modules`, and `.venv`, then uploads it directly to Railway.
...
|---------|----------------------|
| **Local dev** | `.env.local` — `railguey upload-source`, etc. |
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Build railguey
        run: |
          git clone --depth 1 https://github.com/eidos-agi/railguey.git "$RUNNER_TEMP/railguey-source"
          go build -C "$RUNNER_TEMP/railguey-source" -o "$RUNNER_TEMP/railguey" ./cmd/railguey

      - name: Deploy
        env:
          RAILWAY_TOKEN: ${{ secrets.RAILWAY_TOKEN }}
        run: >-
          "$RUNNER_TEMP/railguey" upload-source "$GITHUB_WORKSPACE" "${{ vars.RAILWAY_SERVICE }}" --message "$GITHUB_SHA"
```
