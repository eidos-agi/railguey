...
- **Docs**: New `docs/railway-agent-ssh.md` — full guide to reading about Railway services through the agent over SSH: prerequisites, how navigation works, conversation persistence, JSON shape, troubleshooting, and the distinction from `railway ssh -s <service>` (exec inside your own container). Linked from the README docs and commands tables.
- **Added**: Sibling-token discovery fallback in `_load_project_token`. When a workspace has no `RAILWAY_TOKEN` in its own `.env.local`/`.env`, railguey scans sibling directories under the same parent; if **≥2 siblings agree** on the same value (the safety threshold — project tokens are duplicated across a project's repos by convention, a misfiled personal token is not), it writes the token to the workspace's `.env.local` with a provenance comment and proceeds. A fresh workspace self-bootstraps without an interactive `railguey login`. The no-token error is now a three-path recovery guide (sibling discovery for agents, `railguey login` for humans, env var for CI).
- **Added**: Codex plugin manifest (`.codex-plugin/plugin.json`) and `skills/use-railguey/SKILL.md` — an agent-facing operating guide for the CLI (shipping flow, evidence standard, destructive-verb rules).
- **Tested**: 6 new tests in `tests/test_token.py` covering sibling discovery — inherit at ≥2 siblings, refuse at 1, majority value wins, own `.env.local` beats siblings, provenance comment written, plain `.env` siblings honored. Plus `tests/test_research.py` coverage for the research verb. Fixed an order-dependent flake: `test_empty_workspace_returns_clean_error` used raw `tmp_path`, whose shared parent let the sibling scan find token-bearing dirs left by other tests — it now uses the isolated `workspace` fixture. Full suite: 259 passed, 13 xfailed.
