...
class TestLoadToken:
    ...
    def test_strips_whitespace(self, workspace):
        _write(workspace / ".env.local", "  RAILWAY_TOKEN=  spaced  \n")
    ...
    def test_raises_when_token_missing_from_file(self, workspace):
        _write(workspace / ".env.local", "OTHER_VAR=hello\n")
        with pytest.raises(ValueError, match="RAILWAY_TOKEN not found"):
            _load_token(str(workspace))

    def test_raises_when_token_empty(self, workspace):
        _write(workspace / ".env.local", "RAILWAY_TOKEN=\n")
        with pytest.raises(ValueError, match="RAILWAY_TOKEN not found"):
            _load_token(str(workspace))
    ...
    def test_resolves_tilde_path(self, tmp_path):
        """Ensure expanduser works (won't literally test ~ but exercises the path)."""
        _write(tmp_path / ".env.local", "RAILWAY_TOKEN=tilde-test\n")
        assert _load_token(str(tmp_path)) == "tilde-test"
